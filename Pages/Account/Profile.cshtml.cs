    ...
    public class ProfileModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            try
            {
                ...
                var emailExist = await dbContext.Accounts.FirstOrDefaultAsync(a => a.Email.Equals(Auth.Email) && a.AccountId != auth.AccountId);

                if (emailExist != null)
                {
                    ViewData["success"] = "This email is already in use, please choose another email!";
                    return Page();
                }
